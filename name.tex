...
\mhahn{TODO one thing that has to become a bit clearer is that we are explaining a substantial amount of copmplexity beyond the universals. We could also compare to baselines based on the universals. E.g. in Korean there is a huge amount of variance explained by AUC optimization even though not much happens in terms of the universals.}
...
\textsc{Finnish} & \textsc{Turkish} & \textsc{Hungarian} \\
    ...
    \end{tabular}
    \caption{AUC Histograms for Noun Suffixes: We show a smoothed histogram of baseline orderings, and the AUC values for optimized, real, and reverse orderings. For the real orderings, we also indicate what fraction of baseline grammars have higher AUC, with a Binomial confidence interval. Note that real and optimizesd orders and identical and thus indistinguishible in this figure. \mhahn{could think about making this a small violin plot for each language, instead of providing numerical hard-to-think-about quantile infoemation}}
    \label{fig:auc_nouns}
\end{figure}
