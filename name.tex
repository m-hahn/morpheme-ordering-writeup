...
A key idea of the Memory--Surprisal Tradeoff is that, due to limitations in human memory, human expectations in reality do not reflect the true context $w_1\dots w_{t-1}$, but some memory representation $m\_t$:
\begin{equation}   \label{eq:lossy-surp}
    \text{Difficulty} \propto -\log P(w_t | m_t).
\end{equation}
There is a tradeoff between average surprisal and memory capacity:
...
A key consequence of this theorem is information locality:
...
\end{tabular}
\end{quote}
\textit{Valence} affixes change the number of arguments. A very common type of valence affix is a causative, which adds an argument. In Turkish, -t- marks causatives:
...
Aspect and tense categories are often fused.

TODO example

\textit{Mood} describes a relation between an event and the speaker, including an assessment of the event's reality.
An example is the Potential mood 
...
Mood may also be fused with tense and aspect; for instance, the Japanese suffix -\textit{yoo} can mark future tense and the hortative mood (`let's ...').
Tense, aspect, mood are often together referred to as TAM (Tense-Aspect-Mood).
...
Agreement with the object is found in Sesotho and in Hungarian; it is fused with subject agreement in the latter case.
...
To investigate the memory-surprisal tradeoff of natural languages, we compare the efficency each natural language's morpheme orderings (\textit{real}) to three different kinds of baselines: randomized morpheme orderings (\textit{random}), the reversed natural orderings (\textit{reverse}), and morpheme orderings that are optimized to maximize MI (\textit{optimized}). 
...
\becky{Explain how optimization works.}
\end{itemize}
...
\becky{What's happening with Japanese?}
...
\becky{TODO: discuss each lg, conclude}
