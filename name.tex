...
\mhahn{I'm wondering whether it is necessary to explain the theorem formally, or whether there might be a benefit in being a bit less formal here and only doing a pictorial presentation, referring to the PsychReview paper for the formal detai;s.}
...
\end{figure}
 ...
 \begin{figure}
     ...
     \caption{Verb affix order across the six languages in our sample, matched with four universal categories (left). }
...
Figures~\ref{tab:examples-verbs}--\ref{tab:table-orders} summarise the affixes in the verbal morphologies of the six languages considered here (see SI Section X for details on how we arrived at these summaries).
...
\textit{Valence} affixes (red in Figures~\ref{tab:examples-verbs}--\ref{tab:table-orders}) change the number of arguments. A very common type of valence affix is a causative, which adds an argument indicating who causes an event or state to occur \citep{wals-111}.
\textit{Voice} (green in Figures~\ref{tab:examples-verbs}--\ref{tab:table-orders}) describes the distinction between active and passive (CITE). % An example is the Japanese passive suffix -\textit{(r)are}- (Figure~\ref{tab:examples-verbs}).
\textit{Tense-Aspect-Mood (TAM)} (blue in Figures~\ref{tab:examples-verbs}--\ref{tab:table-orders}) comprises three types of categories.
...
Other languages have multiple slots, for instance, Turkish TAM markers are distributed across two slots (Figure~\ref{tab:table-orders}).

\textit{Subject agreement} (orange in Figure~\ref{tab:table-orders}) mark categories of the subject, most often its person and number, sometimes also other categories such as its gender \citep{corbett2003agreement}.
...
We use the term \textit{Embedding} for this type of morpheme.

Having introduced the two universal generalizations about noun and verb affixes, we now turn to testing the hpothesis that these generalizations arise from optimization for efficient memory-surprisal tradeoffs.
...
We obtained between (TODO) and (TODO) inflected nouns and between (TODO) and (TODO) inflected verbs in each language.
...
Applying any such ordering to the forms extracted from the corpus results in a set of counterfactual forms with some associated memory-surprisal tradeoff curve; in particular, applying the real orderings (Figure \ref{tab:table-orders}) results in the actual morpheme sequences as found in the corpus.
...
\begin{figure}
    ...
    \caption{AUC Histograms for Verb Affixes. \mhahn{TODO add optimized and reverse}}
    \label{fig:auc_verbs}
\end{figure}
...
\multirow{3}{*}{Nouns} & Finnish  & 1.0 (0.0) & 0.42 (0.31) & 1.0 (0.0) & 0.37 (0.32) & 1.0 (0.0) & 0.38 (0.29) \\
 ...
 \hline
\multirow{7}{*}{Verbs} & Finnish & 0.4 (0.3) & 0.4 (0.37) & 0.38 (0.31) & 0.39 (0.38) & 0.36 (0.32) & 0.38 (0.37) \\
...
\begin{minipage}{.3\textwidth}
  ...
  \include{figures/sesotho_suffixes/comparison-opt-uni}
    \end{minipage}
\end{tabular}
    \caption{Comparing optimized orders with the universal order described by \citep{bybee-morphology-1985}. \mhahn{TODO when running the final iteration of the models, merge Aspect/Tense/Mood into TAM for the universal order.}  Note that, for Sesotho prefixes, inversion of order relative to the universal order is expected, as the universal order indicates distance from the root, not absolute position. \mhahn{TODO for Sesotho prefixes, just flip the order in presentation.}}
...
\subsubsection{Grammatical Motivations}
...
\subsubsection{Cognitive Motivations}
