...
\mhahn{I'm pondering to what extent it would be helpful to expound on the connection between the tradeoff and locality, to make the link to Bybee.}
...
Figures \ref{tab:optimized_acc_nouns}--\ref{tab:optimized_acc_verbs} show the agreement of optimized and random baseline orderings with real orderings.
...
\textsc{Finnish} & \textsc{Turkish} & \textsc{Hungarian} \\
    ...
    \end{tabular}
    \caption{AUC Histograms for Noun Suffixes: We show smoothed histograms of baseline orderings (brown) and orderings satisfying the universals (purple), and the AUC values for optimized (red), real (green), and reverse (blue) orderings. For the real orderings, we also indicate what fraction of baseline grammars have higher AUC, with a Binomial confidence interval. Note that real and optimizesd orders and identical and thus indistinguishible in this figure. \mhahn{TODO i'll still make a color legend} }
    \label{fig:auc_nouns}
\end{figure}
...
\includegraphics[width=0.7\textwidth]{figures/accuracies_nouns.pdf}
    ...
    For the baselines, we provide both a smoothed violin plot of the distribution of accuracies, and the mean accuracy. For the optimized order, we show the accuracy. Comparisons between optimized and baseline (both random and based on universals) orderings are significant at $p<0.001$.}
...
\includegraphics[width=0.8\textwidth]{figures/accuracies_verbs.pdf}
    ...
    For the baselines, we provide both a smoothed violin plot of the distribution of accuracies, and the mean accuracy. For the optimized order, we show the accuracy. Except for Finnish, comparisons between optimized and random baseline orderings are significant at $p<0.001$.}
