...
Human language encodes thoughts into linear strings of words, and ultimately sounds (CITE).
...
They also suggested that mutual information formalizes previous notions of strength of semantic association between morphemes \cite{bybee-morphology-1985}. %, such as \cite{bybee-morphology-1985}'s notion of \textit{relevance}.
...
First, we consider data from four additional languages, testing whether it predicts 
...
In this section, we introduce some general crosslinguistic tendencies in morpheme ordering, including discussion of the the six languages.
...
Figure~\ref{fig:noun-inflection} shows a fully inflected Finnish noun, with endings for number (plural), case (illative), and possessor (2nd person singular).

Number and case marking are the subject of a well-documented universal, namely \citep[112]{greenberg1963universals}'s Universal 39:
...
\end{tabular}
\caption{Noun inflection in Finnish}\caption{fig:noun-inflection}
\end{figure}
...
\node[rectangle,text width=1.2cm,anchor=base,color=blue] (A4) at (1,-3) {Aspect};
...
\node[rectangle,text width=1.2cm,anchor=base,color=orange] (B4) at (3,-4) {6lar};
\node[rectangle,text width=1.2cm,anchor=base,color=blue] (B4) at (3,-4.5) {7TAM2};
\node[rectangle,text width=1.2cm,anchor=base,color=orange] (B4) at (3,-5) {8Agreement};
...
%The affixes closest to the verb root are derivational affixes, and inflectional affixes appear further out.
...
While agreement is most commonly established with the subject, agreement with the object is found in Sesotho \cite{doke1967textbook} and in Hungarian \cite{rounds2001hungarian}; it is fused with subject agreement in the latter case.
