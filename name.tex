...
Explanations of the first kind state that morphemes are ordered based on differences in semantic scope \citep{rice2000morpheme} or relevance \citep{bybee-morphology-1985}, so that morphemes with higher relevance or narrower scope are closer to the root.
...
\subsection{Previous Accounts of Morpheme Ordering}
...
This even applies to elements that are clearly scope-bearing, for instance, negation appears closer to the root than TAM in Rukish and farther from it in Sesotho.
...
\includegraphics[width=0.8\textwidth]{figures/accuracies_verbs.pdf}
    ...
    For the baselines, we provide both a smoothed violin plot of the distribution of accuracies, and the mean accuracy. For the optimized order, we show the accuracy.
    In all languages, optimized orderings provide higher accuracy than the mean of the random baselines at $p<0.001$. \mhahn{perhaps also give the numerical quantiles, at least in the main text}}
...
\paragraph{Relation to Syntactic Theories}
...
\paragraph{Relation to Semantic Theories}
