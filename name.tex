...
\author{Michael Hahn, Becky Mathew, Judith Degen}
...
Explanations of the first kind state that morphemes are ordered based on differences in semantic scope \citep{rice2000morpheme} or relevance \citep{bybee-morphology-1985}, so that morphemes are ordered in relation to the closeness of their semantic relationship with the root.
Explanations of the second kind proposes that principles of morpheme ordering mirror principles of word order, due to either language history or synchronic constraints on language.
Explanations of the third kind argue that affixes are closer to the root when they are more likely to be processed together with it.
...
They show that optimizing the memory-surprisal tradeoff amounts to placing elements close together when they predict each other strongly, as measured by mutual information.
...
Languages apply affix moprphemes to different classes of words, including both open word classes such as nouns, verbs, and adjectives, and closed word classes, in particular pronouns.
...
Prominent accounts of morpheme ordering universals are in terms of a correspondence between morpheme ordering and semantics~\citep{bybee-morphology-1985,rice2000morpheme}.
\citet{bybee-morphology-1985} argues ordering is determined by the semantic relevance of affixes to the root.
For example, she argues that morphemes changing a verb's argument structure, such as passives and causatives, have a particularly strong relation to the verb's semantics, as they fundamentally alter the nature of the event described, whereas tense or agreement markers are much less tightly linked the the verb's meaning.
In this account, \textit{relevance} is defined informally and no operationalization is provided.
...
While this account is successful at predicting the order of valence and voice (with the exception of anti-scope orderings in some languages, \citep{...}), it is less straightforwardly evaluated for other affixes, and its predictions can depend on the specifics of how meaning is represented (see SI Section XXX for more on this).
There are also cases where semantically equivalent affixes are ordered differently in different languages, e.g., possessive suffices are ordered differently in Finnish nouns than in Hungarian and Turkish nouns, seemingly without a motivating difference in semantic scope; the scope-based theory makes no prediction about how a given language will order the affixes.
This even applies to elements that are clearly scope-bearing, for instance, negation appears closer to the root than TAM in Turkish and farther from it in Sesotho.
...
However, this principle alone does not directly explain why elements are ordered the way they are in syntax and morphology. \mhahn{TODO think more about what to say here}
...
This theory holds that affixes are closer to the root when they are more likely to be processed together with the base in human lexical access  \citep{baayen1993on}.
This theory has been applied to the order of derivational affixes in English, such as \mhahn{TODO}, but not to the affix ordering generalizations described in Sections~\ref{sec:univ-nouns}--\ref{sec:univ-verbs}.
...
Here, we review the memory-surprisal tradeoff and the Efficient Tradeoff Hypothesis as an explanatory principle of order in language, before we then test the Efficient Tradeoff Hypothesis as a predictor of morpheme order in Section~\ref{seq:testing}.
...
\begin{figure}
    ...
    \begin{tabular}{ccc}
    (A) & (B) \\
    \includegraphics[width=0.5\textwidth]{figures/tradeoff.pdf} & TODO AUC
    \end{tabular}
...
\citet{Hahn2020modeling} show that tradeoffs are more efficient when elements have higher mutual information with other nearby elements.
...
\end{center}
    ...
    \caption{AUC Histograms for Noun Suffixes: We show smoothed histograms of baseline orderings (brown) and orderings satisfying the universals (purple), and the AUC values for optimized (red), real (green), and reverse (blue) orderings. For the real orderings, we also indicate what fraction of baseline grammars have higher AUC, with a binomial confidence interval. Note that real and optimizesd orders are identical and thus indistinguishible in this figure. \mhahn{TODO i'll still make a color legend} \mhahn{do something for the overlap. e.g. semitransparent, or change the colors, and make it clear in legend} }
    \label{fig:auc_nouns}
\end{figure}
...
\begin{figure}
    ...
    \caption{AUC Histograms for Verb Affixes: We show smoothed histograms of baseline orderings (brown) and orderings satisfying the universals (purple), and the AUC values for optimized (red), real (green), and reverse (blue) orderings. For the real orderings, we also indicate what fraction of baseline grammars have higher AUC, with a binomial confidence interval. \mhahn{TODO make a color legend} \mhahn{TODO figure out how to make those labels readblae}}
    \label{fig:auc_verbs}
\end{figure}
...
\begin{minipage}{.3\textwidth}
  ...
  \end{tabular}
    ...
    \mhahn{a possible further feature to consider might be a visual indication of the frequency of different morphemes. The idea being that the model's `errors' tend to concern low-frequency morphemes}
    }
...
We have examined morpheme order in nouns and verbs in six morphologically rich and agglutinating languages, testing the recently proposed Efficient Tradeoff Hypothesis \citep{Hahn2020modeling} as an explanatory account of morpheme ordering.
