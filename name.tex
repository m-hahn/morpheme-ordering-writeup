...
Broadly, this theory holds that affixes are closer to the root when they are less strongly separable from the base.

Baayen's(1993) dual processing race model of morphological access.
Forms are less separable when they are more likely to be processed together with the root in human morphological processing
(Baayen \& Schreuder 1999, Baayen \& Moscoso del Prado Mart ́ın 2005).
...
\cite{hay2001lexical} argues that whether an affix is processed together with the root is determined in part by the relative frequency
\begin{equation}
