...
With this procedure, we identified the slots described below:
\begin{enumerate}
...
Additional morphemes that do not fit into any of the slots and occur less than 50 times are placed into an UNKNOWN slot. %, this affects \textcolor{red}{TODO} \% of morpheme occurrences in the dataset.
...
\caption{Frequent Korean verb suffixes in slots 1-8. The first column describes the nine slots. Second column: Representation of one allomorph in Hangul. Third column: Identifier.}\label{tab:korean-frequent-morphemes}
\end{table}


Next, we illustrate how our segmentation corresponds to commonly described paradigms.
...
Second, in Tables~\ref{tab:korean-hada-1}--\ref{tab:korean-hada-3}, we show an illustrative paradigm for the verb \korean{하다} \textit{hada} `to do' as described in Wiktionary\footnote{Selection of forms and labeling of rows and columns in Tables ~\ref{tab:korean-hada-1}--\ref{tab:kroean-itda} are from \texttt{https://en.wiktionary.org/wiki/\korean{하다}} and \texttt{https://en.wiktionary.org/wiki/\korean{있다}} (retrieved September 16, 2020).}, which covers some of the affixes in slots 4--7.
...
We show these in order to illustrate how commonly used forms map onto morpheme sequences.
...
\multirow{6}{*}{Interrogative} & \multirow{3}{*}{Non-past} & \korean{하느냐} & \korean{해}  & \korean{해요}  & \korean{합니까} \\
 ...
 \hline
	      ...
	      &  & -\textsc{ess}$_4$-\textsc{nya}$_7$ &  -\textsc{ess}$_4$-\textsc{eo}$_7$ &  -\textsc{ess}$_4$-\textsc{eo}$_7$-\textsc{yo}$_8$ &  -\textsc{ess}$_4$-\textsc{p}$_5$-\textsc{ni}$_6$-\textsc{kka}$_7$\\
...
\hline
\end{tabular}
	\caption{Selected forms of \textit{hada} `to do' (see text for further information). In each cell, we provide Hangul  and transliterated (e.g., handa) forms, a representation in terms of underlying morphemes (e.g., -\textsc{n}$_6$-\textsc{da}$_7$), and -- where available -- how the form is segmented in the Kaist corpus (e.g., px+ef).}\label{tab:korean-hada-1} % (e.g., \korean{한다})\korean{하+ㄴ다} and 
\end{table}
...
&& -- & -- & -- & -- \\
    \end{tabular}
    \caption{Continuation of Tables~\ref{tab:korean-hada-1}--\ref{tab:korean-hada-2}: Selected forms of hada with an honorific (\textsc{si}$_3$). We provide Hangul forms, morpheme sequences, and -- where available -- segmentations as given in the Kaist corpus.}
...
&& -\textsc{get}$_4$-\textsc{da}$_7$ & -\textsc{get}$_4$-\textsc{eo}$_7$ & -\textsc{get}$_4$-\textsc{eo}$_7$-\textsc{yo} & -\textsc{get}$_4$-\textsc{p}$_5$-\textsc{ni}$_6$-\textsc{da}$_7$ \\
    ...
    \end{tabular}
    \caption{Selected forms of \textit{itda}. Refer to Table~\ref{tab:korean-hada-1} for details.}
